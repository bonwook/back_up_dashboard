...
export default async function ClientReportViewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 완료된 작업: 당사자(assigned_to, assigned_by) 또는 공동업무 서브태스크 담당자만 접근
  const task = await queryOne<any>(
    `
      ...
      WHERE ta.id = ?
        ...
        AND (ta.assigned_to = ? OR ta.assigned_by = ? OR ta.id IN (SELECT task_id FROM task_subtasks WHERE assigned_to = ?))
      LIMIT 1
    `,
    [id, me.id, me.id, me.id],
  )
